...
namespace Microsoft.Azure.Management.Reservations
{
    ...
    internal partial class ReservationOrderOperations : IServiceOperations<AzureReservationAPIClient>, IReservationOrderOperations
    {
        ...
        /// A response object containing the response body and response headers.
        /// </return>
        public async Task<AzureOperationResponse<ChangeDirectoryResponse>> ChangeDirectoryWithHttpMessagesAsync(string reservationOrderId, ChangeDirectoryRequest body, Dictionary<string, List<string>> customHeaders = null, CancellationToken cancellationToken = default(CancellationToken))
        {
            if (reservationOrderId == null)
            {
                throw new ValidationException(ValidationRules.CannotBeNull, "reservationOrderId");
            ...
            string apiVersion = "2020-11-15-preview";
            // Tracing
            bool _shouldTrace = ServiceClientTracing.IsEnabled;
            string _invocationId = null;
            if (_shouldTrace)
            {
                ...
                tracingParameters.Add("body", body);
                tracingParameters.Add("cancellationToken", cancellationToken);
                ServiceClientTracing.Enter(_invocationId, this, "ChangeDirectory", tracingParameters);
            }
            // Construct URL
            var _baseUrl = Client.BaseUri.AbsoluteUri;
            var _url = new System.Uri(new System.Uri(_baseUrl + (_baseUrl.EndsWith("/") ? "" : "/")), "providers/Microsoft.Capacity/reservationOrders/{reservationOrderId}/changeDirectory").ToString();
            _url = _url.Replace("{reservationOrderId}", System.Uri.EscapeDataString(reservationOrderId));
            List<string> _queryParameters = new List<string>();
            if (apiVersion != null)
            {
            ...
            // Create Result
            var _result = new AzureOperationResponse<ChangeDirectoryResponse>();
            _result.Request = _httpRequest;
            _result.Response = _httpResponse;
            if (_httpResponse.Headers.Contains("x-ms-request-id"))
            {
            ...
            if ((int)_statusCode == 200)
            {
                ...
                try
                {
                    _result.Body = Rest.Serialization.SafeJsonConvert.DeserializeObject<ChangeDirectoryResponse>(_responseContent, Client.DeserializationSettings);
                }
                catch (JsonException ex)
                {
                    _httpRequest.Dispose();
